...
真实请求前确认 `ARK_API_KEY` 已在 skill-local `.env` 或进程环境中配置；不得输出或提交密钥。仓库根目录的 `.env` 不会被 CLI 读取，仅 skill-local `skills/imagegen/.env` 与进程环境生效。agent 创建的 prompt 临时文件统一使用项目根目录 `.seedream-prompt-<random-id>.txt`，其中 `<random-id>` 为 6–64 位 ASCII 字母、数字、`_` 或 `-`，首字符须为字母或数字，并配合 `--cleanup-prompt-file`。不得删除用户输入或不确定请求状态文件。
