...
- [Skill 工作流](skills/imagegen/SKILL.md)：模型选择、生成流程、计费保护和交付规则。
- [Prompt 规则](skills/imagegen/references/prompting.md)与[场景模板](skills/imagegen/references/sample-prompts.md)：由 agent 按需选择结构化 prompt，用户无需填写表单。
- [视觉示例](skills/imagegen/references/visual-examples.md)：可选风格参考，默认不会加入请求。
- [CLI 参考](skills/imagegen/references/cli.md)、[Lite 规范](skills/imagegen/references/lite.md)与[Pro 规范](skills/imagegen/references/pro.md)：命令、参数和模型边界。
- [色键参考](skills/imagegen/references/chroma-key.md)：受校验的透明背景工作流与限制。
...
- 统一依赖文件：`skills/imagegen/requirements.txt` 同时包含运行与测试依赖。
...
当前公开 Ark 页面无法把每个 Model ID 和限制定位到可直接引用的静态正文。[`references/lite.md`](skills/imagegen/references/lite.md) 与 [`references/pro.md`](skills/imagegen/references/pro.md) 是版本化本地约束；修改前必须重新核对官方 Ark 文档。

## 色键边界

色键只适用于平坦、高饱和背景和不含键色色相族的实心主体，不是毛发、烟雾、玻璃、液体、薄纱、运动模糊、软阴影或半透明对象的通用分割工具。受校验命令、alpha 契约、失败规则和三项交付检查见 [`references/chroma-key.md`](skills/imagegen/references/chroma-key.md)。
...
`pyproject.toml` 用于标准化项目元数据和 `pytest` 配置（当前包含测试目录与默认报告）；它不是第二个依赖安装入口。所有依赖只通过 `skills/imagegen/requirements.txt` 安装。
